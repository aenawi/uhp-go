...
func TestIdleTickFiresWhileNothingIsPublished(t *testing.T) {
	run := newRun("resp_idle", "sess_idle", newFeed(0), DefaultTaskBudget, nil, harness.StepEdgeStart, func() {}, func() {})
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func TestIdleTickChangesNoEventOrder(t *testing.T) {
	run := newRun("resp_both", "sess_both", newFeed(0), DefaultTaskBudget, nil, harness.StepEdgeStart, func() {}, func() {})
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func TestIdleTickErrorEndsTheSubscription(t *testing.T) {
	run := newRun("resp_gone", "sess_gone", newFeed(0), DefaultTaskBudget, nil, harness.StepEdgeStart, func() {}, func() {})
	defer run.finish()
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
